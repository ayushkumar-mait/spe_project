...
| --- | --- |
| 1. Application Setup | Job API, Worker, Healing Controller, shared models, JSON logging |
...
The project uses a distributed job processing system:

```text
User -> Job API -> Kafka/Redpanda Queue -> Worker Pods -> Redis Status Store
```
